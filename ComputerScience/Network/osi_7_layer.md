...
1-3. [OSI 7 계층? TCP/IP?](#1-3-osi-7-계층-tcpip)  
1-4. [통신 절차 예시](#1-4-통신-절차-예시)  
...
2-3-5. [정리](#2-3-5-정리)  
2-4. [전송 계층(Transport Layer)](#2-4-전송-계층transport-layer)  
2-4-1. [프로그램(프로세스)간 통신](#2-4-1-프로그램프로세스간-통신)   
2-4-2. [인코딩/디코딩](#2-4-2-인코딩디코딩)  
2-4-3. [기술 구현 위치](#2-4-3-기술-구현-위치)  
2-4-4. [장비의 예](2-4-4-장비의-예)  
2-4-5. [정리](#2-4-5-정리)  
2-5. [세션 계층(Session Layer)](#2-5-세션-계층session-layer)  
2-5-1. [정리](#2-5-1-정리)   
2-6. [표현 계층(Presentation Layer)](#2-6-표현-계층presentation-layer)  
2-6-1. [정리](#2-6-1-정리)  
2-7. [응용 계층(Application Layer)](#2-7-응용-계층application-layer)  
2-7-1. [정리](#2-7-1-정리)  
...
### 1. OSI 7 계층이란? 
  ...
  - #### 1-4. 통신 절차 예시   
    ![image](https://user-images.githubusercontent.com/65080004/116649761-04d55380-a9bb-11eb-8b60-58a191f28bcf.png)  
...
### 2. 단계별로 살펴보는 OSI 7 계층
  ...
  - #### 2-4. 전송 계층(Transport Layer)
    - ##### 2-4-1. 프로그램(프로세스)간 통신 
      - 전송 컴퓨터에서 수신 컴퓨터로 데이터를 보냄(뒤 과정이 있으나 설명이 길어지므로 생략)
      - 현재 데이터를 받는 컴퓨터에 실행중인 프로그램(프로세스)가 있음
      - 수신 컴퓨터는 받은 데이터를 어떤 프로세스에게 줘야할지 어떻게 알 수 있을까?
      - 각 프로세스는 포트번호를 가지고 있음, 이를 통해 데이터를 프로세스에게 전달 가능  
        `포트번호` - 하나의 컴퓨터에서 동시에 실행되고 있는 프로세스들이 서로 겹치지 않게 가져야하는 정수 값  
      - 질문에 대한 과정을 설명한다면 전송 컴퓨터는 데이터 수신 컴퓨터 프로세스의 포트번호를 붙여 보내고,  
        수신 컴퓨터는 받은 데이터를 디코딩하여 포트번호를 확인한 뒤 프로세스에 데이터를 전달함  
    ...
    - ##### 2-4-2. 인코딩/디코딩
      - 과정
        ...
        2. 전송 계층 인코더에서 데이터가 송수신지 포트번호가 포함되어 나옴 
        ...
        5. 라우터에 도착하여 1~4계층 디코더를 통해 해석 된 후 목적지가 맞는지 확인
        
        6. 아닐경우 다시 4~1 계층 순서로 인코딩을 통해 아날로그 신호로 변환
        
        7. 목적지가 맞을 경우 디코더를 통해 해석되어 해당 프로세스에 데이터가 전달 됨
        ```
    - ##### 2-4-3. 기술 구현 위치
      - 운영체제의 커널에
      - 소프트웨어 적으로 구현
    ...
    - ##### 2-4-5. 정리
      - `Port 번호를 사용`하여 도착지 컴퓨터의 최종 목적지인 `프로세스`까지  
        데이터가 도달하게 하는 모듈
      - 즉, 출발지와 목적지간 신뢰할 수 있는 데이터를 전송을 보장해주는 계층 
      - 전송단위  
        `TCP` 일때 `세그먼트(Segment)`  
        `UDP` 일때 `데이타그램(Datagram)`  
      - 오류 검출 및 복구, 흐름제어 기능을 한다.  
        단, UDP는 흐름제어나 오류복구는 하지 않음(프로토콜마다 다를 수 있음)  
      - 흐름제어 - 송수신 호스트의 전송 과정의 속도 차이를 제어해주는 기능
      - 오류제어 - 전송의 유효성 확인, 데이터의 변형, 분실 오류 시 `재전송` 기능으로 복구 수행함으로써 신뢰성있는 통신 보장
      - 참고 (데이터 링크 계층과 전송 계층이 차이)  
        데이터 링크 계층 : 물리적으로 1:1 연결된 호스트 사이의 전송 의미  
        전송 계층 : 논리적으로 1:1 연결된 호스트 사이의 전송 의미  

  - #### 2-5. 세션 계층(Session Layer)  
    - ##### 2-5-1. 정리
      - `응용 프로그램 계층` 간 `통신에 대한 규격이나 접속 설정을 동기화` 하는 계층  
      - 데이터를 전송계층에 `전송할 순서를 정해`주고 점검과 복구를 위한 위치를 제공  
        즉, 통신을 관리하기 위한 방법을 제공해준다.  
      - 통신 방식에는 `동시 송수신`, `반이중`, `전이중` 방식의 통신이 있음
      - `TCP/IP 세션을 만들고 없애는 책임`을 진다.  
  
  - #### 2-6. 표현 계층(Presentation Layer)  
    - ##### 2-6-1. 정리
      - 응용계층의 다양한 표현 양식을 범용적인 전송방식으로 전환하고, 암호화, 복호화, 데이터 압축을 하는 계층  
      - 사용자의 명령어를 완성하고, 그 결과를 표현하고 암호화, 압축, 포장을 한다.   
  
  - #### 2-7. 응용 계층(Application Layer)  
    - ##### 2-7-1. 정리
      - 응용프로세스간의 인터페이스나 통신을 위한 기본적인 응용을 제공하는 계층
      - 사용자로부터 `데이터를 받아 하위 계층으로 넘겨준다.`  
      - 사람이 인식하고 응용 계층 아래의 표현 계층과 상호 작용할 수 있는 데이터와 그림을 사용자에게 보여주는 역할  
